...
public class ForgotController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	{
		...
		if(action.equalsIgnoreCase("submit"))
		{
			...
			if(f==null)
			{
				request.setAttribute("invalidemail", "Email Address not found in Database..");
		...
		else if(action.equalsIgnoreCase("confirm"))
		{
			...
			UserModel f=new UserModel();
			HttpSession session=request.getSession(false);
			f=(UserModel)session.getAttribute("UserData");
			f.setPassword(pswd);
			f.setUid(f.getUid());
			ForgotDao dao=new ForgotDao();
			...
			if(r>0)
			{
				...
				System.out.println("Password Reset is Success!");
			}
			else
			{
