...
<title>View Confirmed Services</title>
...
<body>
	...
	<!-- Shopping Cart Section Begin -->
		<div class="container">
			<div class="row">
				<div class="col-lg-12">
				<h2 style="color:#0A0F4E;">View Confirmed Services</h2>
				<hr>
				<br />
					<div class="shopping__cart__table">
						<table>
							...
							<tbody>
								...
								String qry = "SELECT book.bid,subservices.`image`,subservices.`subsname`,book.`bdate`,book.`bstatus`,book.`pstatus`,serviceman.`firstname`,serviceman.`lastname` FROM assignserviceman INNER JOIN book ON book.`bid`=assignserviceman.`bid` INNER JOIN services ON services.`serviceid`=assignserviceman.`serviceid` INNER JOIN subservices ON subservices.`subid`=assignserviceman.`subid` INNER JOIN serviceman ON serviceman.`servicemanid`=assignserviceman.`servicemanid` INNER JOIN customer ON customer.`customerid`=assignserviceman.`customerid` WHERE customer.`customerid`=? AND book.`bstatus`='confirm' OR book.`bstatus`='assigned'";
